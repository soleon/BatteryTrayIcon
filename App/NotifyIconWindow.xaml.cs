...
public partial class NotifyIconWindow
{
    ...
    private bool _isUpdatingBatteryStatus;
    ...
    private void OnLoaded(object sender, RoutedEventArgs args)
    {
        ...
        _batteryStatusUpdateSubject.Throttle(DebounceTimeSpan).ObserveOn(AsyncOperationManager.SynchronizationContext)
            .Where(_ =>
            {
                lock (_batteryStatusUpdateSubject)
                {
                    return !_isUpdatingBatteryStatus;
                }
            })
    ...
    private void UpdateBatteryStatus()
    {
        try
        {
            lock (_batteryStatusUpdateSubject)
            {
                if (_isUpdatingBatteryStatus) return;
                _isUpdatingBatteryStatus = true;
            }
        ...
        finally
        {
            lock (_batteryStatusUpdateSubject)
            {
                _isUpdatingBatteryStatus = false;
            }
        }
    }
}